 ...
 * @author GIGA
 */
public class ThanhtoanRepositoryImpl {
    public List<Thanhtoan> getChoDo(Map<String,String> params){
        try(Session s = HibernateUtils.getFACTORY().openSession()){
            CriteriaBuilder cb = s.getCriteriaBuilder();
            CriteriaQuery<Thanhtoan> q = cb.createQuery(Thanhtoan.class);
            Root root = q.from(Thanhtoan.class);
            q.select(root);
            
            if(params != null){
                List<Predicate> predicates = new ArrayList<>();
                
                String booking_id = params.get("booking_id");
                if(booking_id !=null && !booking_id.isEmpty())
                    predicates.add(cb.equal(root.get("bookingid").get("id").as(String.class), booking_id));
